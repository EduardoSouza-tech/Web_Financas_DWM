...
export default function DashboardPage() {
  ...
  const inNext7Days = (date?: string) => {
    if (!date) return false;
    const d = new Date(`${date}T12:00:00`);
    return d >= new Date(today.toDateString()) && d <= next7Days;
  };
  const daysUntil = (date: string) =>
    Math.ceil((new Date(`${date}T12:00:00`).getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
  const byDate = (a: string, b: string) => a.localeCompare(b);
  const upcomingDebts = debts.filter(d => inNext7Days(d.nextDueDate)).sort((a, b) => byDate(a.nextDueDate, b.nextDueDate));
  ...
  const getCriticalAlerts = () => {
    ...
    if (isInDeficit) {
      alerts.push({
        ...
        message: `Você está gastando ${formatCurrency(Math.abs(availableAfterDebts))} a mais do que ganha ao considerar dívidas e despesas.`,
    ...
    if (debtCommitmentPercentage > 30) {
      alerts.push({
        id: 'debt-high',
        type: 'warning',
        title: 'Endividamento Alto',
        message: `${debtCommitmentPercentage.toFixed(1)}% da sua renda está comprometida com dívidas (recomendado: máx 30%)`,
    ...
    if (totalGoalsMonthly > availableAfterDebts && availableAfterDebts >= 0) {
      alerts.push({
        ...
        message: `Você precisa de ${formatCurrency(totalGoalsMonthly)}/mês para suas metas, mas tem apenas ${formatCurrency(availableAfterDebts)} disponível`,
        action: 'Ajuste as metas ou aumente a receita',
        priority: 'medium'
      });
    } else if (totalGoalsMonthly > 0 && availableAfterDebts < 0) {
      alerts.push({
        id: 'goals-impossible',
        type: 'danger',
        title: 'Metas Financeiras Impossíveis',
        message: `Suas metas exigem ${formatCurrency(totalGoalsMonthly)}/mês, mas você está em déficit de ${formatCurrency(Math.abs(availableAfterDebts))}`,
        action: 'Resolva o déficit antes de focar em metas',
        priority: 'high'
      });
    }
  ...
  return (
    <div className="space-y-8">
      ...
      >
          ...
          <Card className="glass border-red-500/20 hover:border-red-500/40 transition-all">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {monthlyIncome > 0 ? ((monthlyExpenses / monthlyIncome) * 100).toFixed(1) : 0}% da receita
              </p>
            </CardContent>
          </Card>
        </motion.div>
          ...
          <Card className="glass border-orange-500/20 hover:border-orange-500/40 transition-all">
            ...
            <CardContent>
              ...
              <p className="text-xs text-orange-600 font-semibold mt-1">
                {debtCommitmentPercentage.toFixed(1)}% da renda comprometida
              </p>
              {debtCommitmentPercentage > 30 && (
        ...
        <motion.div variants={itemVariants}>
          <Card className={`glass transition-all ${availableAfterDebts < 0 ? 'border-red-500/40' : 'border-primary/20 hover:border-primary/40'}`}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Saldo Real Mensal</CardTitle>
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${availableAfterDebts < 0 ? 'bg-red-500/10' : 'bg-primary/10'}`}>
                {availableAfterDebts < 0 ? (
            ...
            <CardContent>
              <div className={`text-3xl font-bold ${availableAfterDebts < 0 ? 'text-red-600' : 'text-green-600'}`}>
                {formatCurrency(availableAfterDebts)}
              </div>
              <p className={`text-xs font-semibold mt-1 flex items-center gap-1 ${availableAfterDebts < 0 ? 'text-red-600' : 'text-muted-foreground'}`}>
                {availableAfterDebts < 0 ? (
                  <>
                    <ArrowDownRight className="h-3 w-3" />
                    Déficit de {Math.abs(realSavingsRate).toFixed(1)}%
                  </>
                ) : (
                  <>
                    <ArrowUpRight className="h-3 w-3" />
                    Taxa: {realSavingsRate.toFixed(1)}%
                  </>
                )}
              </p>
              {availableAfterDebts < 0 && (
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="border-t pt-2">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Taxa de poupança: {monthlyIncome > 0 ? ((availableAfterExpenses / monthlyIncome) * 100).toFixed(1) : 0}% {availableAfterExpenses > 0 ? '✅ Excelente!' : ''}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Badge variant={isInDeficit ? 'danger' : 'default'}>
                    {isInDeficit ? 'Negativo' : 'Positivo'}
                  </Badge>
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                  ...
                  <div className="border-t pt-2">
                    <div className="flex justify-between">
                      ...
                      <span className={`font-bold ${isInDeficit ? 'text-red-600' : 'text-green-600'}`}>
                        = {formatCurrency(availableAfterDebts)}
                      </span>
                    </div>
                    {isInDeficit && (
                      <p className="text-xs text-red-600 font-semibold mt-1 flex items-center gap-1">
                        ...
                        Você está no vermelho! Déficit de {formatCurrency(Math.abs(availableAfterDebts))} por mês
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="p-4 bg-blue-50 dark:bg-blue-950/10 border border-blue-200 dark:border-blue-800 rounded-lg">
                  ...
                  <p className="text-sm text-blue-800 dark:text-blue-200">
                    {(() => {
                      ...
                      const top2Debts = sortedDebts.slice(0, 2);
                      const freedMoney = top2Debts.reduce((sum, d) => sum + d.monthlyPayment, 0);
                      
                      if (top2Debts.length === 2) {
                        return `Priorize quitar "${top2Debts[0].name}" (${top2Debts[0].interestRate}% a.m. de juros) para liberar ${formatCurrency(top2Debts[0].monthlyPayment)}/mês. Depois, foque em "${top2Debts[1].name}" (${top2Debts[1].interestRate}% a.m.) para liberar mais ${formatCurrency(top2Debts[1].monthlyPayment)}/mês. Ao quitar essas duas dívidas, você terá ${formatCurrency(availableAfterDebts + freedMoney)}/mês ${availableAfterDebts + freedMoney > 0 ? 'positivos' : 'de déficit reduzido'}!`;
                      } else if (top2Debts.length === 1) {
                        return `Priorize quitar "${top2Debts[0].name}" (${top2Debts[0].interestRate}% a.m. de juros) para liberar ${formatCurrency(top2Debts[0].monthlyPayment)}/mês e melhorar sua situação financeira!`;
                      }
                      return 'Reduza despesas e aumente sua receita para sair do vermelho.';
                    })()}
                  </p>
                </div>
              )}
      ...
      >
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {generateInsights(health, debts, getExpensesByCategory()).map((insight) => (
            <motion.div
              key={insight.id}
      ...
      >
        <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
          ...
          <Badge variant="warning" className="ml-2">
            {upcomingDebts.length + upcomingCards.length + upcomingSubscriptions.length} próximos
          </Badge>
        </h2>
        ...
        <div className="grid gap-4 md:grid-cols-3">
            ...
            <CardContent className="space-y-3">
              {(() => {
                ...
                return upcoming.map(debt => {
                  const dueDate = new Date(`${debt.nextDueDate}T12:00:00`);
                  ...
                  return (
                    <div key={debt.id} className="p-3 bg-red-50 dark:bg-red-950/20 rounded-lg border border-red-200 dark:border-red-800">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <Badge variant={days <= 2 ? 'danger' : 'warning'} className="text-xs">
            ...
            <CardContent className="space-y-3">
              {(() => {
                ...
                return upcoming.map(card => {
                  const dueDate = new Date(`${card.nextDueDate}T12:00:00`);
          ...
          <Card className="glass border-blue-500/20">
            ...
            <CardContent className="space-y-3">
              {(() => {
                const upcoming = upcomingSubscriptions;
                ...
                return upcoming.map(sub => {
                  const renewDate = new Date(`${sub.nextPayment}T12:00:00`);
      ...
      {achievements.length > 0 && (
        ...
        >
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            ...
            <Button variant="ghost" size="sm" className="ml-auto text-muted-foreground" onClick={handleClearAchievements}>
              Limpar conquistas
            </Button>
          </h2>
                  ...
                  <CardContent className="space-y-2">
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div className="p-2 bg-blue-100 dark:bg-blue-900/30 rounded">
                        <p className="text-xs text-muted-foreground">Liberado por mês</p>
      ...
      >
        ...
        <Card className="glass">
          ...
          <CardContent className="space-y-6">
            {(() => {
              const newIncome = monthlyIncome + incomeBoost;
              const newExpenses = monthlyExpenses - expenseReduction;
              const newAvailable = newIncome - newExpenses - monthlyDebtPayments;
              const newSavingsRate = newIncome > 0 ? ((newAvailable / newIncome) * 100) : 0;
              const improvement = newAvailable - availableAfterDebts;
                    ...
                    <div className="p-4 bg-gradient-to-r from-primary/10 to-purple-500/10 rounded-lg border-2 border-primary/30">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div>
                          ...
                          <p className={`text-xs ${newSavingsRate >= realSavingsRate ? 'text-green-600' : 'text-red-600'}`}>
                            {newSavingsRate >= realSavingsRate ? '↑' : '↓'}
                            {Math.abs(newSavingsRate - realSavingsRate).toFixed(1)}pp
                          </p>
                        </div>
                      </div>

                      {newAvailable > 0 && isInDeficit && (
